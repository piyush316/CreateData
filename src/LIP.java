...
public class LIP {
    ...
    public static String readshort;
    public static String read(String filename){
        ...
        try{
            ...
            Return[] rt ={new Return (1,0.0010), new Return(2,0.0015), new Return(5,0.0020), new Return(10,0.0025),  new Return(30,0.0035), new Return(60,0.0050)};
            ...
            while(cr.readRecord()){
                if(cr.get(2).length()<2){
                	System.out.println("Error in date format in "+filename);
                }
                Date time=dateFormat.parse(cr.get(2));
                int t=(time.getHours()*60)+(time.getMinutes());
                double vwappr=Double.parseDouble(cr.get(3));
                hm.put(t,vwappr);
            }
          ...
          //  double prevniftyclose=ClosingPrices.niftyprevdayclose(filename.substring(filename.lastIndexOf("/")+1,filename.length()-4), "/home/pyrole/Strategies/Close/NIFTY_PIYUSHFINAL", nift.expiry());
           // double prevstockclose=ClosingPrices.stockprevDayClose(filename.substring(filename.lastIndexOf("/")+1,filename.length()-4), "/home/pyrole/Strategies/Close/Stocks/"+StockName);
            ...
            while(cr.readRecord()){
                ...
                String print_long="";
                String print_short="";
                System.out.println(filename+" "+cr.get(2));
                Date time=dateFormat.parse(cr.get(2));
                int t=(time.getHours()*60)+(time.getMinutes());
                //print=print+" "+filename+":"+t;
                double voo=Double.parseDouble(cr.get(4));
                int  vol=new Double(voo).intValue();
                double vwappr=Double.parseDouble(cr.get(3));
                //double high=Double.parseDouble(cr.get(6));
                //double low=Double.parseDouble(cr.get(7));
                //int ntrades=vol;//;Integer.parseInt(cr.get(3));
                ...
                rs.update(vwappr);
                print=print+" "+rs.getRetSince(5)+" "+rs.getRetSince(15)+" "+rs.getRetSince(35)+" "+rs.getRetSince(156);
...
*/
               ...
               // print = print +" "+prevniftyclose;
                ...
                //print =print + " "+ ClosingPrices.returnsfromClose(nift.price(t), prevniftyclose,filename);
                ...
                for (int i=0;i<niftyreturns.length;i++){
                   ...
                   System.out.println(niftyreturns[i].getReturns());
                }
                
         
                if(hm.get(t+1)!=null && hm.get(t+predictiontime)!=null ){
                	
                    String print1=print+" "+highTarget(hm, t+1);
                    ...
                    readlong=readlong+print1.trim()+"\n";
                    readshort=readshort+print2.trim()+"\n";
    ...
    public static String highTarget(HashMap<Integer,Double> h, int time){
       ...
       // System.out.println(h.get(time)+" "+h.get(time+29));
        ...
        if ((1.0005*(h.get(time)))<h.get(time+predictiontime-1))
        {r="HIGH";}
        return r;
    }
    ...
    public static String lowTarget(HashMap<Integer,Double> h, int time){
        ...
        if ((0.9995*(h.get(time)))>h.get(time+predictiontime-1))
        {r="LOW";}
        return r;
    }
        ...
        for(int i=0; i+training_period+testing_period<=filenames.length;i=i+testing_period){
            try{
                ...
                for (int j=i;j<i+training_period;j++){
                	LIP.read(filenames[j]); 
                	//System.out.println(readlong);
                    trainlong=trainlong+readlong;
                    trainshort=trainshort+readshort;
                }
                for(int j2=i+training_period;j2<i+training_period+testing_period;j2++){
                    LIP.read(filenames[j2]);
                    testlong=testlong+readlong;
                    testshort=testshort+readshort;
                }
                p.print(trainlong);
